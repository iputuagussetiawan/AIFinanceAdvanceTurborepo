...
export const InstitutionService = {
    getInstitutions: async () => {
        ...
        return institutions.map(({ _id, ...rest }) => ({ id: _id.toString(), ...rest }))
    },

    getInstitutionsPaginated: async (
        page: number = 1,
        limit: number = 10,
        search?: string,
        type?: string,
        ...
        const [data, totalData] = await Promise.all([
            InstitutionModel.find(query).select('-__v').sort({ orderPosition: 1, name: 1 }).skip(skip).limit(limit).lean(),
    ...
    createInstitution: async (body: InstitutionDTO) => {
    ...
    bulkCreateInstitution: async (institutions: InstitutionDTO[]) => {
        const session = await mongoose.startSession()
        try {
            session.startTransaction()

            const names = institutions.map((inst) => inst.name)
            const existingInstitutions = await InstitutionModel.find({ name: { $in: names } }).session(session)
    ...
    updateInstitution: async (id: string, body: UpdateInstitutionDTO) => {
        if (!mongoose.Types.ObjectId.isValid(id)) throw new BadRequestException('Invalid Institution ID format')
    ...
    getInstitutionById: async (id: string): Promise<IInstitution> => {
        if (!mongoose.Types.ObjectId.isValid(id)) throw new BadRequestException('Invalid Institution ID format')

        const institution = await InstitutionModel.findById(id).select('-__v').lean().exec()
        if (!institution) throw new NotFoundException('Institution record not found')
