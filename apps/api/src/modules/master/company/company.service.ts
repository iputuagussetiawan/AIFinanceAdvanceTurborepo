...
export const CompanyService = {
    createCompany: async (data: ICompanyInput): Promise<CompanyDocument> => {
        ...
        if (existingSlug) throw new BadRequestException('Company slug is already in use')
    ...
    bulkCreateCompany: async (companies: ICompanyInput[]) => {
    ...
    updateCompany: async (companyId: string, updateData: ICompanyUpdate): Promise<CompanyDocument> => {
        ...
        if (updateData.slug && updateData.slug !== company.slug) {
            ...
            if (slugExists) throw new BadRequestException('The new slug is already taken')
        }
    ...
    getCompanyBySlug: async (slug: string): Promise<CompanyDocument> => {
    ...
    searchCompanies: async (search: string = '') => {
        const filter: any = { isActive: true }
        if (search) {
            filter.$or = [
                { name: { $regex: search, $options: 'i' } },
