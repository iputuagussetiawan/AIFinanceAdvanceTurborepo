...
export const LanguageService = {
    searchLanguages: async (search: string = '', priority?: number) => {
        const filter: FilterQuery<LanguageDocument> = { isActive: true }
        if (search) filter.name = { $regex: search, $options: 'i' }
        if (priority !== undefined) filter.priority = priority

        const data = await LanguageModel.find(filter).select('-__v').sort(DEFAULT_SORT)
        return data.map((doc) => doc.toJSON())
    },

    getAllLanguages: async (isActive?: boolean) => {
        const query: FilterQuery<LanguageDocument> = {}
        if (typeof isActive === 'boolean') query.isActive = isActive

        const rawItems = await LanguageModel.find(query).select('-__v').sort(DEFAULT_SORT).lean()
        return {
            success: true,
    ...
    getLanguagesPaginated: async (page: number = 1, limit: number = 10, search?: string, isActive?: boolean) => {
        ...
        const [rawItems, totalData] = await Promise.all([
            LanguageModel.find(query).select('-__v').sort(DEFAULT_SORT).skip(skip).limit(limit).lean(),
            LanguageModel.countDocuments(query),
        ])

        return {
            data: rawItems.map(toDTO),
            meta: { totalData, totalPage: Math.ceil(totalData / limit), currentPage: page, limit },
        }
    },

    createLanguage: async (body: LanguageDTO) => {
        const existing = await LanguageModel.findOne({ name: new RegExp(`^${body.name}$`, 'i') }).lean()
        if (existing) throw new ConflictException(`Language "${body.name}" already exists`)
    ...
    bulkCreateLanguage: async (languages: LanguageDTO[]) => {
        const session = await mongoose.startSession()
        try {
            session.startTransaction()
            ...
            await session.commitTransaction()
            return { data: savedLanguages, count: savedLanguages.length }
        } catch (error: any) {
            await session.abortTransaction()
            throw error
        } finally {
    ...
    updateLanguage: async (id: string, body: UpdateLanguageDTO) => {
        if (!mongoose.Types.ObjectId.isValid(id)) throw new BadRequestException('Invalid Language ID format')
        ...
        if (body.name) {
            ...
            if (conflict) throw new ConflictException(`Language "${body.name}" already exists`)
        }
    ...
    getLanguageById: async (id: string): Promise<LanguageDocument> => {
        if (!mongoose.Types.ObjectId.isValid(id)) throw new BadRequestException('Invalid Language ID format')

        const language = await LanguageModel.findById(id).select('-__v').lean()
        if (!language) throw new NotFoundException('Language record not found')
