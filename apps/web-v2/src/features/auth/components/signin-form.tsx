...
import { DASHBOARD_URL, SIGNUP_URL, FORGOT_PASSWORD_URL } from '@/lib/constants'
import { authService } from '../services/auth-service'
import { signinSchema, type SigninInput } from '../types/auth-type'
...
export function SignInForm() {
    ...
    const { mutate, isPending } = useMutation({
        mutationFn: (data: SigninInput) => authService.login(data),
        onSuccess: () => {
    ...
    return (
        <form onSubmit={handleSubmit((d) => mutate(d))} className="flex flex-col gap-6">
            ...
            <div className="flex flex-col gap-4">
                ...
                <div className="flex flex-col gap-1.5">
                    <div className="flex items-center">
                        ...
                        <Link href={FORGOT_PASSWORD_URL} className="ml-auto text-xs text-muted-foreground hover:underline">
                            Forgot password?
                        </Link>
                    </div>
            ...
            <p className="text-center text-sm text-muted-foreground">
                No account?{' '}
                <Link href={SIGNUP_URL} className="font-medium text-foreground hover:underline">
                    Sign up
                </Link>
            </p>
        </form>
